...
namespace Charipay.Application.Interfaces.Repositories
{
    public interface IVolunteerUserRepository : IRepository<VolunteerUser>
    {
        ...
        Task<(List<MyVolunteerApplicationDto> Items, int TotalCount)>  GetMyApplicationsAsync(Guid UserId,int pageNumber, int pageSize, string? search = null, string? status = null);
